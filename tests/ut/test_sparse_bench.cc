...
knowhere::DataSetPtr
LoadSparseEmbeddings(const std::string& file_path) {
    ...
    // LOG_KNOWHERE_INFO_ << "total nnz: " << ttl_nnz << std::endl;
...
TEST_CASE("Test Mem Sparse Index MsMarco BM25 index", "[float metrics]") {
    ...
    std::vector<const char*> metrics = {knowhere::metric::IP, knowhere::metric::BM25};
    ...
    float used_avgdl = GENERATE(100.0f, 12.7782);
    ...
    for (auto topk : topks) {
        for (auto metric : metrics) {
            ...
            // auto gt = knowhere::BruteForce::SearchSparse(docs, queries, conf, nullptr);
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - now);
            LOG_KNOWHERE_INFO_ << "[metric " << metric << " topk " << topk << " avgdl " << used_avgdl << " refine_factor " << refine_factor << "]" << std::endl;// BruteForce search time: " << duration.count() << "ms" << std::endl;
            ...
            for (auto index_name : index_names) {
                ...
                for (auto drop_ratio_build : drop_ratio_builds) {
                    ...
                    for (auto drop_ratio_search : drop_ratio_searchs) {
                        ...
                        LOG_KNOWHERE_INFO_ << "start search" << std::endl;
                        // for (int i = 0; i < 100; i++) {
